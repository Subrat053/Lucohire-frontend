...
const AdminRenewalPolicy = () => {
  ...
  const fetchContent = async () => {
    try {
      const { data } = await adminAPI.getContent('renewal');
      setContent(data || '');
    } catch (err) {
      toast.error('Failed to load renewal policy');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await adminAPI.updateContent('renewal', content);
      toast.success('Renewal Policy updated!');
    } catch (err) {
      toast.error('Failed to update renewal policy');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Renewal Policy Editor</h1>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        <textarea
          ...
          placeholder="Enter renewal policy content here..."
        />
...
export default AdminRenewalPolicy;
