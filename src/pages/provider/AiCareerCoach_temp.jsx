...
export default function AiCareerCoach() {
  ...
  if (loading || !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
        ...
        <p className="text-lg text-gray-500 font-medium">Your AI coach is preparing your dashboard...</p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-lg font-medium">Ask me anything about your career journey.</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {['Improve My Resume', 'Find Better Jobs', 'Prepare for Interview', 'Salary Tips'].map((action, i) => (
            <button key={i} onClick={() => handleQuickChip(action)}
              className="px-3 py-1 bg-white border border-gray-200 hover:border-[#0f766e] hover:text-[#0f766e] hover:bg-teal-50/50 rounded-full text-lg font-bold transition-all">
              {action}
            </button>
          ))}
        </div>
          ...
          <div className="mb-3 space-y-2 max-h-52 overflow-y-auto pr-1">
            {chatHistory.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] px-3 py-2 rounded-xl text-lg font-medium ${msg.role === 'user' ? 'bg-[#0f766e] text-white' : 'bg-gray-50 border border-gray-100 text-gray-800'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
            {chatLoading && (
              <div className="flex justify-start">
                <div className="bg-gray-50 border border-gray-100 px-3 py-2 rounded-xl text-lg text-gray-500 flex items-center gap-2">
                  <div className="w-4 h-4 border-2 border-[#0f766e] border-t-transparent rounded-full animate-spin" /> Thinking...
                </div>
              </div>
            )}
            <div ref={chatEndRef} />
        ...
        <form onSubmit={handleChatSubmit} className="relative">
          <input
            ...
            className="w-full pl-4 pr-12 py-2.5 rounded-xl border border-gray-200 focus:border-[#0f766e] focus:outline-none text-lg font-medium transition-all"
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex justify-between items-center mb-3">
          <h3 className="text-lg font-bold flex items-center gap-1.5">
            ...
            <span className="text-lg font-semibold text-gray-400 ml-1">({completedCount}/{tasks.length})</span>
          </h3>
          <button onClick={() => setActiveTab('Daily Tasks')}
            className="text-lg font-bold text-[#0f766e] hover:text-teal-800 flex items-center gap-1 transition-colors">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <div>
                <p className="text-lg font-bold text-gray-500 uppercase tracking-wider">Target Role</p>
                <p className="font-extrabold text-[#0f766e] text-lg">{currentGoal.role || '—'}</p>
              </div>
              <div>
                <p className="text-lg font-bold text-gray-500 uppercase tracking-wider">Timeline</p>
                <p className="font-extrabold text-[#0f766e] text-lg">{currentGoal.time || '—'}</p>
              </div>
              <button onClick={() => setGoalModalOpen(true)}
                className="flex items-center gap-1 px-3 py-1.5 bg-white border border-teal-200 hover:border-[#0f766e] text-[#0f766e] font-bold text-lg rounded-lg transition-all">
          ...
          <div className="flex-1 sm:border-l border-teal-200/50 sm:pl-4 w-full">
            ...
            <p className="text-lg font-bold text-gray-500 mt-1">{currentGoal.completion || 0}% Completed</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold flex items-center gap-1.5">
            ...
            <span className="text-lg text-gray-400 font-semibold ml-1">({completedCount}/{tasks.length} done)</span>
          </h3>
          <button onClick={handleTasksRefresh} disabled={refreshingTasks}
            className="flex items-center gap-1 text-lg font-bold text-[#0f766e] hover:text-teal-800 disabled:opacity-50 transition-colors">
        ...
        {tasks.length === 0 && (
          <p className="text-center text-lg text-gray-400 py-6">No tasks yet. Loading...</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="xl:col-span-2 space-y-4">
      ...
      <div className="bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-100 rounded-2xl p-4">
        ...
        <div className="flex gap-6 mb-3">
          <div>
            <p className="text-lg font-bold text-gray-500 uppercase">Target Role</p>
            <p className="font-extrabold text-[#0f766e] text-lg">{currentGoal.role}</p>
          </div>
          <div>
            <p className="text-lg font-bold text-gray-500 uppercase">Timeline</p>
            <p className="font-extrabold text-[#0f766e] text-lg">{currentGoal.time}</p>
          </div>
          <div>
            <p className="text-lg font-bold text-gray-500 uppercase">Progress</p>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <h3 className="text-lg font-bold mb-4">Career Roadmap</h3>
        <div className="relative pl-5 space-y-4">
          ...
          {careerRoadmap.map((step, i) => (
            <div key={i} className="flex items-start justify-between relative z-10 pl-6">
              ...
              <div>
                <p className={`font-bold text-lg ${i === 0 ? 'text-gray-900' : 'text-gray-500'}`}>{step.title}</p>
                {step.timeframe && <p className="text-lg text-gray-400">{step.timeframe}</p>}
              </div>
              <span className={`px-2 py-0.5 text-lg font-bold rounded-md shrink-0 ${step.status === 'Current' ? 'bg-teal-50 text-[#0f766e]' : step.status === 'Next Step' ? 'bg-indigo-50 text-indigo-600' : 'bg-gray-50 text-gray-500'}`}>
                {step.status}
              </span>
            </div>
          ))}
      ...
      {/* Milestones */}
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <h3 className="text-lg font-bold mb-3">Milestones</h3>
        <div className="space-y-2">
  ...
  const renderResources = () => (
    <div className="xl:col-span-2 space-y-4">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <h3 className="text-lg font-bold mb-3 flex items-center gap-1.5"><BookOpen className="w-6 h-6 text-amber-500" /> Recommended for You</h3>
            ...
            <div key={idx} className="p-3 bg-white border border-gray-100 rounded-2xl hover:shadow-md hover:border-teal-100 transition-all cursor-pointer group"
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-lg font-bold text-gray-900 leading-tight mb-1">{rec.title}</p>
                  <p className="text-lg text-gray-500 line-clamp-2">{rec.description}</p>
                  <div className="flex items-center gap-1 text-[#0f766e] font-bold text-lg mt-2">
      ...
      <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4">
        <h3 className="text-lg font-bold flex items-center gap-1.5 mb-2 text-amber-800">
          <Sparkles className="w-5 h-5 text-amber-500" /> Daily Tip from AI Coach
        </h3>
        <p className="text-lg font-medium text-amber-900 leading-relaxed mb-3">
          {dailyTip || 'A strong portfolio with 3-4 case studies can increase your profile views by up to 60%.'}
        </p>
        <div className="flex items-center justify-between pt-2 border-t border-amber-200">
          <span className="text-lg font-bold text-amber-700">Was this helpful?</span>
  ...
  const renderProgress = () => (
    <div className="xl:col-span-2 space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div key={i} className={`${stat.color} border rounded-2xl p-3 text-center`}>
            ...
            <p className="text-lg font-bold text-gray-500">{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      {/* Overall Progress */}
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <h3 className="text-lg font-bold mb-4">Overall Career Progress</h3>
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1">
            <p className="text-lg font-bold text-gray-900 mb-1">You're making great progress! 🚀</p>
            <p className="text-lg text-gray-500 mb-3">Keep completing daily tasks to boost your score.</p>
            <div className="space-y-2">
              {[
                ...
                <div key={i}>
                  <div className="flex justify-between text-lg font-bold text-gray-500 mb-0.5">
      ...
      {aiInsights.length > 0 && (
        <div className="bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-100 rounded-2xl p-4 shadow-sm">
          <h3 className="text-lg font-bold mb-3 flex items-center gap-1.5 text-indigo-900">
          ...
          <div className="space-y-3">
            {aiInsights.map((insight, idx) => (
              <div key={idx} className="flex gap-3 bg-white/60 p-3 rounded-xl border border-indigo-50/50">
                ...
                <div>
                  <p className="text-lg font-bold uppercase tracking-wider mb-0.5 text-indigo-500">{insight.type}</p>
                  <p className="text-lg font-medium text-indigo-950">{insight.message}</p>
                </div>
              </div>
            ))}
          </div>
  ...
  const renderSidebar = () => (
    <div className="space-y-4">
      {/* Coach Summary */}
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <h3 className="text-lg font-bold mb-3">Coach Summary</h3>
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h4 className="font-bold text-[#0f766e] text-lg">Overall Progress</h4>
            <p className="text-lg text-gray-500 leading-tight">You're on the right track!</p>
            <button onClick={() => setActiveTab('Progress')}
              className="text-lg font-bold text-[#0f766e] flex items-center gap-0.5 mt-1">
        ...
        <div className="grid grid-cols-4 gap-1 text-center">
          {[
            { v: progressStats.tasksCompleted || 0, l: 'Tasks', icon: <CheckCircle2 className="w-6 h-6 text-green-500 mx-auto mb-0.5" /> },
            { v: progressStats.interviewsPracticed || 0, l: 'Interviews', icon: <Video className="w-6 h-6 text-purple-500 mx-auto mb-0.5" /> },
            { v: progressStats.skillsImproved || 0, l: 'Skills', icon: <Award className="w-6 h-6 text-blue-500 mx-auto mb-0.5" /> },
            { v: progressStats.streakDays || 0, l: 'Streak', icon: <Clock className="w-6 h-6 text-orange-500 mx-auto mb-0.5" /> },
          ].map((s, i) => (
            <div key={i} className="flex flex-col items-center">
              {s.icon}
              <span className="text-lg font-extrabold text-gray-900">{s.v}</span>
              <span className="text-lg font-bold text-gray-500">{s.l}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={() => setShowCalendar(v => !v)}
            className="text-lg font-bold text-[#0f766e] flex items-center gap-1 hover:text-teal-800">
            {showCalendar ? 'Hide' : 'Calendar'} <Calendar className="w-4 h-4" />
          </button>
        </div>

        {showCalendar && (
          <div className="mb-3 p-3 bg-teal-50 rounded-xl border border-teal-100 text-lg">
            <p className="font-bold text-teal-800 mb-2">July 2026</p>
            <div className="grid grid-cols-7 gap-1 text-center text-lg font-bold text-gray-500">
              {['S','M','T','W','T','F','S'].map((d, i) => <span key={i}>{d}</span>)}
              {Array.from({length: 31}, (_, i) => (
                <button key={i}
                  className={`aspect-square flex items-center justify-center rounded-md text-lg font-bold
                    ${[18, 20].includes(i+1) ? 'bg-[#0f766e] text-white' : 'hover:bg-teal-100 text-gray-700'}`}>
                  {i + 1}
                </button>
              ))}
            </div>
            <p className="text-lg text-teal-700 mt-2 font-medium">🟢 Sessions on 18th & 20th</p>
          </div>
        )}
            ...
            <div key={i} className="flex items-center justify-between p-2.5 bg-gray-50 border border-gray-100 rounded-xl">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-white rounded-lg text-indigo-500 border border-gray-100">
                  {session.title?.toLowerCase().includes('interview') ? <Video className="w-5 h-5" /> : <FileText className="w-5 h-5" />}
                </div>
                <div>
                  <h4 className="font-bold text-lg text-gray-900 leading-tight">{session.title}</h4>
                  <p className="text-lg text-gray-500">{session.date}</p>
                </div>
              </div>
              <button
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <div className="flex justify-between items-center mb-3">
          <h3 className="text-lg font-bold">Career Roadmap</h3>
          <button onClick={() => setActiveTab('Career Plan')} className="text-lg font-bold text-[#0f766e] flex items-center gap-1">
        ...
        <div className="relative pl-3 space-y-3">
          ...
          {careerRoadmap.map((step, i) => (
            <div key={i} className="flex items-center justify-between relative z-10 pl-7">
              ...
              <span className={`font-bold text-lg ${i === 0 ? 'text-gray-900' : 'text-gray-500'}`}>{step.title}</span>
              <span className={`px-2 py-0.5 text-lg font-bold rounded-md ${step.status === 'Current' ? 'bg-teal-50 text-[#0f766e]' : step.status === 'Next Step' ? 'bg-indigo-50 text-indigo-600' : 'bg-gray-50 text-gray-500'}`}>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        ...
        <p className="text-lg text-gray-700 leading-relaxed mb-3">{dailyTip}</p>
        <div className="flex items-center justify-between pt-2 border-t border-gray-100">
          <span className="text-lg font-bold text-gray-500">Helpful?</span>
      ...
      <div className="mb-4">
        <div className="flex justify-between items-center gap-2">
          <div>
            ...
            <p className="text-gray-500 text-lg mt-0.5">Your personal AI coach to help you grow and land your dream role.</p>
          </div>
          <button onClick={() => setSettingsOpen(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 rounded-lg hover:border-gray-300 font-semibold text-lg transition-colors shadow-sm">
        ...
        <div className="flex gap-6 mt-3 border-b border-gray-100 overflow-x-auto scrollbar-hide">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`pb-2 text-lg font-bold whitespace-nowrap transition-colors ${activeTab === tab ? 'text-[#0f766e] border-b-2 border-[#0f766e] -mb-[1px]' : 'text-gray-500 hover:text-gray-700'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setGoalModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={handleGoalSave} disabled={goalLoading}
              className="w-full mt-4 py-2.5 bg-[#0f766e] hover:bg-teal-800 text-white font-bold text-lg rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      {settingsOpen && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-end" onClick={() => setSettingsOpen(false)}>
          <div className="bg-white h-full sm:h-auto sm:rounded-2xl shadow-2xl p-6 w-full max-w-xs sm:mr-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                <p className="text-lg font-bold text-gray-700 mb-2">Current Goal</p>
                <div className="p-3 bg-teal-50 rounded-xl border border-teal-100 text-lg">
              ...
              <button onClick={() => { setSettingsOpen(false); setGoalModalOpen(true); }}
                className="w-full py-2 border-2 border-[#0f766e] text-[#0f766e] font-bold text-lg rounded-xl hover:bg-teal-50 transition-colors flex items-center justify-center gap-2">
    ...
    <div className={`flex items-center justify-between p-2.5 bg-gray-50/50 hover:bg-gray-50 border border-gray-100 rounded-xl transition-colors group ${task.status === 'completed' ? 'opacity-70' : ''}`}>
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <p className={`font-bold text-lg truncate ${task.status === 'completed' ? 'line-through text-gray-400' : 'text-gray-900'}`}>{task.title}</p>
          {expanded && <p className="text-lg text-gray-500 truncate">{task.description}</p>}
        </div>
      </div>
      {task.status === 'completed'
        ? <span className="px-2 py-0.5 bg-teal-50 text-[#0f766e] font-bold text-lg rounded-lg shrink-0 ml-2">Done</span>
        : <button onClick={() => onToggle(idx)} className="ml-2 px-3 py-1 bg-white border border-gray-200 group-hover:border-[#0f766e] text-gray-700 group-hover:text-[#0f766e] font-bold text-lg rounded-lg transition-all shrink-0">Finish</button>
      }
    </div>
  );
}
