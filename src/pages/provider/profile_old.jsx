...
const TIER_SKILL_ALLOWLIST = {
  'unskilled': new Set([
    'labour', 'helper plumber', 'helper electrician', 'construction helper', 'cleaning helper',
    'painter helper', 'carpenter helper', 'loader', 'delivery helper', 'kitchen helper',
  ]),
  'semi-skilled': new Set([
    'plumber', 'ac mechanic', 'ac technician', 'electrician', 'carpenter', 'painter',
    'mason', 'driver', 'cook', 'tailor', 'machine operator', 'technician',
  ]),
  'skilled': new Set([
    'web developer', 'app developer', 'digital marketer', 'seo expert', 'graphic designer',
    'video editor', 'content writer', 'ai developer', 'cyber security expert', 'senior electrician',
    'interior designer', 'project manager', 'consultant',
  ]),
};

function isSkillAllowedForTier(skill, tier) {
  const normalizedSkill = String(skill || '').trim().toLowerCase();
  const normalizedTier = String(tier || 'unskilled').trim().toLowerCase();
...
const ProviderProfile = () => {
  ...
  const addLocation = (loc) => {
    ...
    if (form.locations.length >= maxLocations) {
      if (plan === 'free') {
        ...
        toast.error('Free tier is limited to 1 location. Redirecting to plans...');
      } else {
  ...
  const handleTierChange = (nextTier) => {
    setForm((prev) => ({
      ...prev,
      tier: nextTier,
      skills: prev.skills.filter((skill) => isSkillAllowedForTier(skill, nextTier)),
    }));
  ...
  const handleAISuggest = async () => {
    ...
    try {
      const payload = {
        ...
        preferredLocation: form.location || form.locations?.[0] || form.city || '',
      ...
      setAiMeta({
        ...
        detectedLocation: profile.suggestedLocations?.[0]?.city || profile.suggestedLocations?.[0]?.formattedAddress || '',
        skills: profile.suggestedSpecialities?.map((item) => item.name) || [],
        suggestedPricingRange: profile.pricing ? `₹${profile.pricing.perHour} / ₹${profile.pricing.perDay} / ₹${profile.pricing.perMonth}` : '',
  ...
  const applyAiSuggestion = () => {
    ...
    const suggestedSpecialities = Array.isArray(aiSuggestion.suggestedSpecialities) ? aiSuggestion.suggestedSpecialities : [];
    const suggestedLocations = Array.isArray(aiSuggestion.suggestedLocations) ? aiSuggestion.suggestedLocations : [];
    const nextMode = aiSuggestion.mode === 'append' ? 'append' : 'replace';

    const nextSkills = nextMode === 'append'
      ? [...new Set([...form.skills, ...suggestedSpecialities.map((item) => item.name)])]
      : suggestedSpecialities.map((item) => item.name);
    const nextLocations = nextMode === 'append'
    ...
    const pricingValue = aiSuggestion.pricing
      ? String(Math.round(Number(
        selectedPricingType === 'daily' ? aiSuggestion.pricing.perDay :
          selectedPricingType === 'monthly' ? aiSuggestion.pricing.perMonth :
  ...
  const applyPricingSuggestion = () => {
    ...
    const amount = selectedType === 'daily'
  ...
  const handleSave = async (e) => {
    ...
    if (!form.tier || !['unskilled', 'semi-skilled', 'skilled'].includes(form.tier)) {
      return toast.error('Please select a skill tier (Skill tier is mandatory)');
    }
            ...
            <div className="lg:col-span-2">
              <ProviderAIChat
                ...
                missingFields={(() => {
                  ...
                  if (!form.description || String(form.description).trim().length < 20) missing.push('Profile Bio / Description');
                  return missing;
                })()}
