...
import {
  HiTrendingUp, HiUsers, HiBriefcase, HiCog, HiChevronLeft, HiChevronRight, HiLogout, HiMenu, HiX, HiClock, HiDocumentText, HiCurrencyRupee,
} from 'react-icons/hi';
...
const navItems = [
  ...
  { label: 'Post a Job',   path: '/recruiter/post-job',     icon: HiBriefcase },
  { label: 'Applications', path: '/recruiter/applications',  icon: HiDocumentText },
  { label: 'History',      path: '/recruiter/history',       icon: HiClock },
  { label: 'Plans',        path: '/recruiter/plans',         icon: HiCurrencyRupee },
  ...
  const SidebarContent = ({ onNavClick }) => (
    <div className="flex flex-col h-full">
      ...
      <div className={`flex items-center px-4 py-5 border-b border-gray-100 ${collapsed ? 'justify-center' : 'space-x-3'}`}>
        <div className="w-8 h-8 bg-linear-to-br from-green-500 to-blue-600 rounded-lg flex items-center justify-center shrink-0">
          <HiBriefcase className="text-white w-4 h-4 " />
        </div>
        {!collapsed && <span className="font-bold text-gray-800 text-sm">Recruiter Panel</span>}
      </div>
    ...
    <div className="flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between bg-white border-b border-gray-100 px-4 py-3">
          ...
          <span className="font-semibold text-gray-800 text-sm md:hidden">Recruiter Panel</span>
