...
const Navbar = () => {
  ...
  // Impersonation State
  const [impersonatorToken, setImpersonatorToken] = useState(localStorage.getItem('impersonatorToken'));
  const [impersonatorRole, setImpersonatorRole] = useState(localStorage.getItem('impersonatorRole'));
  const [impersonatorRestriction, setImpersonatorRestriction] = useState(localStorage.getItem('impersonatorRestriction'));
  ...
  const handleReturnToAdmin = () => {
    if (impersonatorToken) {
      ...
      const roleStr = impersonatorRole === 'manager' ? 'partner' : (impersonatorRole || 'admin');
      window.location.href = `/${roleStr}/dashboard`;
    }
  };
  ...
  return (
    <>
      ...
      {impersonatorToken && (
        <div className="bg-amber-600 text-white px-4 py-2 flex items-center justify-between z-[60] sticky top-0 shadow-md">
          <div className="flex items-center space-x-2">
            <HiLockOpen className="w-5 h-5" />
            <span className="text-sm font-semibold">
              You are currently impersonating a user{impersonatorRestriction === 'payment' ? ' (Restricted to Payment Settings)' : impersonatorRestriction === 'manager_support' ? ' (Restricted to Profile & Jobs)' : ''}.
            </span>
          </div>
          <button
            onClick={handleReturnToAdmin}
            className="bg-white text-amber-700 px-3 py-1 text-xs font-bold rounded-lg shadow-sm hover:bg-amber-50 transition-colors"
          >
            Return to {impersonatorRole === 'manager' ? 'Manager' : 'Admin'} Dashboard
          </button>
        </div>
      )}

      <nav className={`bg-white shadow-sm border-b border-gray-100 sticky ${impersonatorToken ? 'top-10' : 'top-0'} z-50`}>
