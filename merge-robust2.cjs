...
if (heroStartIdx === -1 || afterDualCardsIdx === -1) {
  console.error("Could not find hero or dual cards in LandingPage.jsx.bak");
  process.exit(1);
}
...
if (!talentDataMatch) {
  console.error("Could not find TOP_TALENT in LandingPage.jsx.bak");
  process.exit(1);
}
...
if (cardsContainerStart === -1 || cardsContainerEnd === -1) {
  console.error("Could not find Hourly Cards section in LandingPage.jsx.bak");
  process.exit(1);
}
...
const firstSectionStart = mergedCode.indexOf('<section className="bg-[#F7F9FC] py-16 lg:py-24">');
...
// Replace the first section with `heroAndDualCards` + `topTalentSection`
mergedCode = mergedCode.substring(0, firstSectionStart) + heroAndDualCards + '\n' + topTalentSection + '\n' + mergedCode.substring(secondSectionStart);

fs.writeFileSync('src/pages/LandingPage.jsx', mergedCode);
console.log('Merge complete directly to LandingPage.jsx');
